...
To open a notebook, you need to serve the files in your `iodide` folder with two localhost servers (this required because, for security reasons, we Iodide needs to load resources from two separate origins). By default, these should use ports 5000 and 5555. If you have Python installed, the simplest way to do this is two open two terminals, and run `cd {path to your iodide folder}/iodide; python -m SimpleHTTPServer 5000` in the first terminal, and `cd {path to your iodide folder}/iodide; python -m SimpleHTTPServer 5555` in the second. Then, open `http://localhost:5000/dev/` in your browser. This should should show all the files in your `dev/` folder. Navigating to `http://localhost:5000/dev/iodide.dev.html` will open a blank notebook.
