...
export const typedArrayType = arr =>
...
export default {
  shouldHandle: (value, inContainer) =>
    !inContainer && (Array.isArray(value) || isTypedArray(value)),
  ...
  render: value => {
    ...
    let arrayElements;
    if (len > 0) {
      arrayElements = _.range(len > MAX_ELTS ? HALF_MAX_ELTS : len - 1).map(
        i => (
      ...
      if (len > MAX_ELTS) {
        ...
        arrayElements = arrayElements.concat(
          _.range(len - HALF_MAX_ELTS, len - 1).map(i => (
            <span key={`array_elt_${i}`} title={`array index: ${i}`}>
              {renderValue(value[i], true)}
              {", "}
            </span>
      ...
      arrayElements = <span key="array_elt_empty" title="empty array" />;
    }
    return (
