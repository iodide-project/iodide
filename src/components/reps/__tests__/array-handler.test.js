...
describe("dateHandler shouldHandle", () => {
  ...
  it("accepts array objects", () => {
    ...
    expect(arrayHandler.shouldHandle(new Uint8ClampedArray([0, 257]))).toBe(
      true
    );
    expect(arrayHandler.shouldHandle(new Int16Array([-32768, 32767]))).toBe(
      true
    );
    expect(arrayHandler.shouldHandle(new Uint16Array([0, 65535]))).toBe(true);
    expect(
      arrayHandler.shouldHandle(new Int32Array([-2147483648, 2147483647]))
    ).toBe(true);
    expect(
      arrayHandler.shouldHandle(new Float32Array([-230430430.2343, 20]))
    ).toBe(true);
    expect(
      arrayHandler.shouldHandle(
        new Float64Array([-230430430.2343, 20, 1230230230.2343])
      )
    ).toBe(true);
  });
});
