...
import { FixedPositionContainerUnconnected, mapStateToProps } from '../fixed-position-container'
...
describe('FixedPositionContainerUnconnected mapStateToProps', () => {
  ...
  it('default case', () => {
    expect(mapStateToProps(state, ownProps))
      .toEqual({ style: pane1Style })
  })

  it('if hidden, should have display none', () => {
    ownProps.hidden = true
    expect(mapStateToProps(state, ownProps).style.display)
      .toEqual('none')
  })
