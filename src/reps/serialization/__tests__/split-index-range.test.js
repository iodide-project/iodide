...
describe("split ranges correctly", () => {
  it("the max of each range is one less than min of the next", () => {
    const ranges = splitIndexRange(newRangeDescriptor(0, 100), 10);
  ...
  it("the first min and the last max are correct", () => {
    const MIN = 422;
    const MAX = 34742;
    const ranges = splitIndexRange(newRangeDescriptor(MIN, MAX), 10);
...
describe("split ranges correctly in seeming error cases", () => {
  [
    {
      range: newRangeDescriptor(5, 9994, "ARRAY_RANGE"),
      targetNumRanges: 10
    },
    {
      range: newRangeDescriptor(5, 9994, "ARRAY_RANGE"),
