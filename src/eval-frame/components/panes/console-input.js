...
export class ConsoleInputUnconnected extends React.Component {
  ...
  onLastLine() {
    const { currentLine, totalLines } = getTextAreaPosition(this.textAreaRef.current)
...
const connectMessagePassers = (WrappedComponent, messageFns) => (
  class extends React.Component {
    render() {
      return (
        <WrappedComponent {...this.props} {...messageFns} />
      )
    }
  }
)
...
export const ConsoleInputMessagePasser = connectMessagePassers(
  ...
  {
    updateConsoleText: consoleText => postActionToEditor({
      type: 'UPDATE_CONSOLE_TEXT',
      consoleText,
    }),
    consoleHistoryStepBack: consoleCursorDelta => postActionToEditor({
...
export default connect(mapStateToProps, mapDispatchToProps)(ConsoleInputMessagePasser)
