...
fooblock contents`
  it('jsmdSample, has correct number of chunks', () => {
    expect(jsmdParser(jsmdSample).length)
      .toEqual(2)
  })
...
foo`,
  ...
  it('jsmdSample, has correct chunk eval flags', () => {
    expect(jsmdParser(jsmdSample).map(c => c.evalFlags))
      .toEqual([[], ['show', 'hide', 'render', 'skip']])
  })

  it('jsmdSample, has correct line numbers', () => {
    expect(jsmdParser(jsmdSample).map(c => [c.startLine, c.endLine]))
      .toEqual([
        [0, 2],
        [3, 4],
      ])
...
`,
  ...
  it('jsmdSample, has correct chunk eval flags', () => {
    expect(jsmdParser(jsmdSample).map(c => c.evalFlags))
      .toEqual([[], [], ['show', 'hide', 'render', 'skip'], [], [], ['foo', 'bat']])
  })
